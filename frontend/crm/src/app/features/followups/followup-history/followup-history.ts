...
@Component({
  ...
  templateUrl: './followup-history.html'
})
export class FollowupHistoryComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.isAllowedRole) {
      this.error = 'Not authorized to view follow-up history.';
  ...
  load(): void {
    ...
    this.loading = true;
    this.error = null;
    this.reportSvc.getFollowupStatusUpdates({
      from: this.fromDate,
      to: this.toDate,
      limit: 1000
    }).subscribe({
      ...
      error: (err) => {
        console.error('Follow-up history load failed', err);
        this.error = 'Failed to load follow-up history.';
        this.loading = false;
      }
    });
  }
